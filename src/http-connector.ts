  ...
  private mergeOptions(
    ...
    const headers = new Headers({
      ...
      ...(typeof options?.headers === 'function'
        ? options.headers(endpoint, options.context)
        : (options?.headers ?? {})),
    });
    ...
    if (options?.next || this.defaultOptions.next) {
      const tags =
        typeof options?.next?.tags === 'function'
          ? options.next.tags(endpoint)
          : options?.next?.tags;
      merged.next = {
  ...
  private async request<T>(endpoint: string, options?: RequestOptions): Promise<T> {
    ...
    try {
      ...
      const parseResponse =
        ...
        ((res: Response) => this.defaultParseResponse<T>(res, options?.stream ?? false));
